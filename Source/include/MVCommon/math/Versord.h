...
namespace MVCommon
{
  ...
  /// \brief A rotational quaternion (i.e. versor) with double-precision floating-point values.
	struct Versord final
  {
  public:
    ...
    MVCOMMON_API static Versord FromString(String const & str);
    ...
    MVCOMMON_API static Versord FromRawBytes(ByteArray & bytes, bool consumeBytes = false);
    ...
    MVCOMMON_API static Versord FromElementsVector(Vector4d const & elements);
    ...
    MVCOMMON_API static Versord FromRawElements(double const * elements);
    ...
    /// \returns      a versor
    MVCOMMON_API static Versord CreateRotationAroundAxis(Vector3d const & axis, double angle);
    /// \brief Creates a versor from a rotational part of transformation matrix.
    /// \param matrix a matrix to extract the rotation from
    /// \returns      a versor
    MVCOMMON_API static Versord CreateRotationFromMatrix(Matrix4x4d const & matrix);
    ...
    /// \returns            a versor
    MVCOMMON_API static Versord CreateRotationFromEulerAnglesZYX(Vector3d const & eulerAngles);
    ...
    /// \cond
    MVCOMMON_API Versord & operator*=(Versord const & other);
    MVCOMMON_API friend bool operator==(Versord const & lhs, Versord const & rhs);
    /// \endcond

    DECLARE_CLASS_SPECIFIC_NEW_DELETE(MVCOMMON_API)

    /// \cond
    friend struct VersordHasher;
    /// \endcond
  ...
  /// \cond
  MVCOMMON_API bool operator==(Versord const & lhs, Versord const & rhs);
  MVCOMMON_API bool operator!=(Versord const & lhs, Versord const & rhs);
  MVCOMMON_API Versord operator*(Versord const & lhs, Versord const & rhs);
  MVCOMMON_API Vector3d operator*(Versord const & versor, Vector3d const & vector);
  /// \endcond

  /// \brief A hasher for Versord objects so they can be used in unordered collections.
  struct VersordHasher
  {
