...
namespace MVCommon
{
  /// \brief A 3-dimensional vector with double-precision floating-point values.
  ...
  public:
    ...
    /// \returns    a vector
    MVCOMMON_API static Vector3d FromString(String const & str);
    ...
    MVCOMMON_API static Vector3d FromRawBytes(ByteArray & bytes, bool consumeBytes = false);
    ...
    MVCOMMON_API static double Dot(Vector3d const & lhs, Vector3d const & rhs);
    ...
    MVCOMMON_API static Vector3d Cross(Vector3d const & lhs, Vector3d const & rhs);
    ...
    /// \cond
    MVCOMMON_API Vector3d & operator+=(Vector3d const & other);
    MVCOMMON_API Vector3d & operator-=(Vector3d const & other);
    MVCOMMON_API Vector3d & operator*=(double factor);
    MVCOMMON_API Vector3d & operator/=(double divisor);
    /// \endcond
  ...
  /// \cond
  MVCOMMON_API Vector3d operator-(Vector3d const & vector);
  MVCOMMON_API Vector3d operator+(Vector3d const & lhs, Vector3d const & rhs);
  MVCOMMON_API Vector3d operator-(Vector3d const & lhs, Vector3d const & rhs);
  MVCOMMON_API Vector3d operator*(Vector3d const & vector, double factor);
  MVCOMMON_API Vector3d operator*(double factor, Vector3d const & vector);
  MVCOMMON_API Vector3d operator/(Vector3d const & vector, double divisor);
  MVCOMMON_API bool operator==(Vector3d const & lhs, Vector3d const & rhs);
  MVCOMMON_API bool operator!=(Vector3d const & lhs, Vector3d const & rhs);
  /// \endcond

  /// \brief A hasher for Vector3d objects so they can be used in unordered collections.
