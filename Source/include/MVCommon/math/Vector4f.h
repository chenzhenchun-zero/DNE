...
namespace MVCommon
{
  /// \brief A 4-dimensional vector with single-precision floating-point values.
  ...
  public:
    ...
    /// \returns    a vector
    MVCOMMON_API static Vector4f FromString(String const & str);
    ...
    MVCOMMON_API static Vector4f FromRawBytes(ByteArray & bytes, bool consumeBytes = false);
    ...
    MVCOMMON_API static float Dot(Vector4f const & lhs, Vector4f const & rhs);
    ...
    /// \cond
    MVCOMMON_API Vector4f & operator+=(Vector4f const & other);
    MVCOMMON_API Vector4f & operator-=(Vector4f const & other);
    MVCOMMON_API Vector4f & operator*=(float factor);
    MVCOMMON_API Vector4f & operator/=(float divisor);
    /// \endcond
  ...
  /// \cond
  MVCOMMON_API Vector4f operator-(Vector4f const & vector);
  MVCOMMON_API Vector4f operator+(Vector4f const & lhs, Vector4f const & rhs);
  MVCOMMON_API Vector4f operator-(Vector4f const & lhs, Vector4f const & rhs);
  MVCOMMON_API Vector4f operator*(Vector4f const & vector, float factor);
  MVCOMMON_API Vector4f operator*(float factor, Vector4f const & vector);
  MVCOMMON_API Vector4f operator/(Vector4f const & vector, float divisor);
  MVCOMMON_API bool operator==(Vector4f const & lhs, Vector4f const & rhs);
  MVCOMMON_API bool operator!=(Vector4f const & lhs, Vector4f const & rhs);
  /// \endcond

  /// \brief A hasher for Vector4f objects so they can be used in unordered collections.
  struct Vector4fHasher
  {
